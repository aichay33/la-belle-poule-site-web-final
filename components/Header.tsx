...
const links = [
  ...
  { href: "/recherche-terrain", label: "Terrain" },
  { href: "/systeme-verger", label: "Verger" },
  { href: "/vente-directe", label: "Vente directe" },
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-3 md:px-8">
        <Link href="/" className="flex items-center gap-3" aria-label="Retour à l'accueil La Belle Poule">
          ...
          <div className="leading-tight">
            ...
            <p className="hidden text-[0.65rem] font-bold uppercase tracking-[0.18em] text-terracotta sm:block">Future ferme en préparation</p>
          </div>
        </Link>
        ...
        <nav className="hidden items-center gap-2 lg:flex">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full px-4 py-2 text-sm font-semibold text-green-deep transition hover:bg-cream-soft hover:text-terracotta"
            >
              {link.label}
            </Link>
          ))}
